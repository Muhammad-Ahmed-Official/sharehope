...
import { ArrowRight, Users, Building2, Bot } from "lucide-react";
...
export default  function HeroSection() {
  return (
    <section className="relative h-screen md:h-[74vh] flex items-center justify-center overflow-hidden">
      ...
      <div className="container mx-auto p-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-card border border-border rounded-full px-4 py-2 mb-4 shadow-soft animate-fade-in">
            <Bot className="w-4 h-4 text-secondary" />
            <span className="text-sm font-medium text-muted-foreground">AI-Powered Giving Platform</span>
          </div>

          {/* Main Heading */}
          <h1 className="font-display text-3xl lg:text-5xl font-bold text-foreground mb-6 animate-slide-up">
            Smart Donations {" "}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 animate-slide-up" style={{ animationDelay: "0.3s" }}>
            {donationRecord.map((stat, index) => (
              <div key={index} className="bg-card/60 backdrop-blur-sm border border-border/50 rounded-2xl p-4 md:px-6 shadow-soft">
