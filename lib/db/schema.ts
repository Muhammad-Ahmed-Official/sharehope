...
export const NgoUsers = pgTable("NgoUsers", {
...
export const DonationTable = pgTable("Donation", {
    id: uuid("id").primaryKey().defaultRandom(),
    donarId: uuid("donarId").notNull().references(() => DonarTable.id),
...
export const DonationCauseTable = pgTable("DonationCause", {
   id: uuid("id").primaryKey().defaultRandom(),
   donationId: uuid("donationId").notNull().references(() => DonationTable.id),
